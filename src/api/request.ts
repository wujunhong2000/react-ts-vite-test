...
axios.interceptors.response.use(
    (response) => {
        ...
        Toast.show({
            icon: 'fail',
            content: data || response.statusText || 'Error',
        })
        ...
        if (error.response && error.response.status) {
            switch (error.response.status) {
                ...
                case 404:
                    Toast.show({
                        icon: 'fail',
                        content: `请求不存在${error.response.data?.msg}` || `请求不存在Error`,
                    })
                    break;
                case 406:
                    Toast.show({
                        icon: 'fail',
                        content: `请求参数有误${error.response.data?.msg}` || `请求参数有误Error`,
                    })
                    break;
                default:
                    Toast.show({
                        icon: 'fail',
                        content: `请求错误${error.response.data?.msg}` || `请求错误Error`,
                    })
            }
        }
